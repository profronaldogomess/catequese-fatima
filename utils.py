...
def adicionar_cabecalho_diocesano(pdf, titulo, etapa=""):
    if os.path.exists("logo.png"):
        # Logo movido para baixo para não cortar
        pdf.image("logo.png", 10, 12, 22)
    
    pdf.set_xy(35, 12); pdf.set_font("helvetica", "B", 11); pdf.set_text_color(65, 123, 153)
    ...
    pdf.ln(8); y_topo = pdf.get_y()
    pdf.set_fill_color(248, 249, 240); pdf.rect(10, y_topo, 100, 18, 'F'); pdf.rect(10, y_topo, 100, 18)
    pdf.set_xy(12, y_topo + 4); pdf.set_font("helvetica", "B", 11); pdf.set_text_color(0, 0, 0)
    pdf.multi_cell(95, 5, limpar_texto("Ficha de Inscrição da Catequese com Inspiração Catecumenal"))
    
    pdf.set_xy(115, y_topo); pdf.set_font("helvetica", "B", 12)
    pdf.cell(35, 18, limpar_texto(f"Ano: {date.today().year}"), border=1, align='C')
    
    pdf.set_xy(150, y_topo); pdf.set_font("helvetica", "B", 7)
    pdf.multi_cell(50, 9, limpar_texto(f"Etapa: {etapa}\nTurma: {etapa}"), border=1)
    pdf.ln(6)
...
def gerar_ficha_cadastral_catequizando(dados):
    ...
    # Turno com marcação de X
    pdf.set_font("helvetica", "B", 10)
    turno_val = str(dados.get('turno', '')).upper()
    mark_m = "X" if "MANHÃ" in turno_val or "M" == turno_val else " "
    mark_t = "X" if "TARDE" in turno_val or "T" == turno_val else " "
    mark_n = "X" if "NOITE" in turno_val or "N" == turno_val else " "
    ...
    pdf.set_xy(100, y + 4); pdf.set_font("helvetica", "B", 8); pdf.cell(20, 4, limpar_texto("Batizado:"), ln=0)
    marcar_opcao(pdf, "Sim", dados.get('batizado_sn') == 'SIM', 120, y + 4)
    marcar_opcao(pdf, "Não", dados.get('batizado_sn') == 'NÃO', 140, y + 4)
