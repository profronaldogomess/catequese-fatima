# ==============================================================================
# ARQUIVO: utils.py
# VERSÃO: 2.0.0 - ESTABILIDADE ABSOLUTA E INTEGRIDADE TOTAL
...
def _desenhar_corpo_ficha(pdf, dados):
    """Lógica central de desenho da ficha com consentimento LGPD rigoroso."""
    y_base = pdf.get_y()
...
def gerar_ficha_cadastral_catequizando(dados):
    pdf = FPDF(); pdf.add_page(); adicionar_cabecalho_diocesano(pdf, titulo=""); _desenhar_corpo_ficha(pdf, dados)
...
# ==============================================================================
# 5. RELATÓRIOS EXECUTIVOS (DIOCESANO V4, PASTORAL V3 E SACRAMENTAL)
# ==============================================================================

def gerar_relatorio_diocesano_v4(dados_censo, equipe_stats, sac_ano, sac_censo, logistica_lista, formacoes_lista, analise_ia):
    """Relatório Diocesano de Alta Precisão v4 com exclusão de ADMIN e segmentação clara."""
    ...
    pdf.cell(190, 8, limpar_texto("2. DADOS SACRAMENTAIS DA EQUIPE (QUALIFICAÇÃO)"), ln=True, fill=True, align='C')
    pdf.set_font("helvetica", "B", 8); pdf.set_text_color(0, 0, 0); pdf.set_fill_color(230, 230, 230)
    ...
    pdf.set_font("helvetica", "", 9); total_e = int(dados_censo.get('total_equipe', 1))
    ...
    pdf.cell(190, 8, limpar_texto(f"3. SACRAMENTOS: CELEBRAÇÕES {date.today().year} VS CENSO ATUAL"), ln=True, fill=True, align='C')
    pdf.set_font("helvetica", "B", 7); pdf.set_text_color(0, 0, 0); pdf.set_fill_color(230, 230, 230)
    pdf.cell(50, 7, "Sacramento", border=1, fill=True); pdf.cell(35, 7, "Realiz.(Inf)", border=1, fill=True, align='C'); pdf.cell(35, 7, "Realiz.(Adu)", border=1, fill=True, align='C'); pdf.cell(35, 7, "Censo(Inf)", border=1, fill=True, align='C'); pdf.cell(35, 7, "Censo(Adu)", border=1, fill=True, align='C'); pdf.ln()
    pdf.set_font("helvetica", "", 8)
    for n, rk, ra, ck, ca in [("BATISMO", sac_ano.get('bat_k', 0), sac_ano.get('bat_a', 0), sac_censo.get('bat_k', 0), sac_censo.get('bat_a', 0)), ("EUCARISTIA", sac_ano.get('euca_k', 0), sac_ano.get('euca_a', 0), sac_censo.get('euca_k', 0), sac_censo.get('euca_a', 0)), ("CRISMA", "N/A", sac_ano.get('crisma_a', 0), "N/A", sac_censo.get('crisma_a', 0))]:
        pdf.cell(50, 7, f" {n}", border=1); pdf.cell(35, 7, str(rk), border=1, align='C'); pdf.cell(35, 7, str(ra), border=1, align='C'); pdf.cell(35, 7, str(ck), border=1, align='C'); pdf.cell(35, 7, str(ca), border=1, align='C'); pdf.ln()
    pdf.ln(5); pdf.set_fill_color(65, 123, 153); pdf.set_text_color(255, 255, 255)
    pdf.cell(190, 8, limpar_texto("4. LOGÍSTICA, LOCAIS E DIAS DE ENCONTRO"), ln=True, fill=True, align='C')
    pdf.set_font("helvetica", "B", 8); pdf.set_text_color(0, 0, 0); pdf.set_fill_color(230, 230, 230)
    ...
    else: pdf.cell(190, 6, "Nenhuma logística mapeada.", border=1, align='C', ln=True)
    pdf.ln(5); pdf.set_fill_color(65, 123, 153); pdf.set_text_color(255, 255, 255)
    pdf.cell(190, 8, limpar_texto(f"5. CRONOGRAMA DE FORMAÇÕES REALIZADAS EM {date.today().year}"), ln=True, fill=True, align='C')
    pdf.set_font("helvetica", "B", 8); pdf.set_text_color(0, 0, 0); pdf.set_fill_color(230, 230, 230)
    pdf.cell(30, 7, "Data", border=1, fill=True, align='C'); pdf.cell(100, 7, "Tema", border=1, fill=True); pdf.cell(60, 7, "Formador", border=1, fill=True); pdf.ln()
    pdf.set_font("helvetica", "", 8)
    if formacoes_lista:
        for f in formacoes_lista: pdf.cell(30, 6, formatar_data_br(f['data']), border=1, align='C'); pdf.cell(100, 6, limpar_texto(f['tema']), border=1); pdf.cell(60, 6, limpar_texto(f['formador']), border=1); pdf.ln()
    else: pdf.cell(190, 6, "Nenhuma formação registrada.", border=1, align='C', ln=True)
    pdf.ln(5); pdf.set_fill_color(224, 61, 17); pdf.set_text_color(255, 255, 255)
    pdf.cell(190, 8, limpar_texto("6. PARECER TÉCNICO E DIRETRIZES PASTORAIS"), ln=True, fill=True, align='C')
...
def gerar_relatorio_pastoral_v3(turmas_detalhadas, totais_gerais, analise_ia):
    """Relatório Pastoral Analítico v3 com detalhamento nominal por turma."""
    ...
    for t in turmas_detalhadas:
        ...
        y_antes = pdf.get_y(); pdf.multi_cell(45, 3.5, limpar_texto(f"{t['nome']}\n{t['catequista']}"), border=1)
        y_depois = pdf.get_y(); h_linha = y_depois - y_antes
        pdf.set_xy(55, y_antes); pdf.multi_cell(25, h_linha/2 if h_linha > 7 else h_linha, limpar_texto(f"{t['dia']}\n{t['local']}"), border=1)
        pdf.set_xy(80, y_antes); pdf.cell(20, h_linha, str(t['batizados']), border=1, align='C')
    ...
    pdf.ln(5); pdf.set_fill_color(65, 123, 153); pdf.set_text_color(255, 255, 255); pdf.set_font("helvetica", "B", 10)
    ...
    desenhar_campo_box(pdf, "Freq. Média Paroquial", f"{totais_gerais['freq_geral']}%", 154, y, 46); pdf.ln(18)
...
def gerar_relatorio_sacramentos_tecnico_pdf(stats, analise_turmas, analise_ia):
    """Auditoria Sacramental Técnica."""
    pdf = FPDF(); pdf.add_page(); adicionar_cabecalho_diocesano(pdf, "AUDITORIA SACRAMENTAL")
    pdf.multi_cell(190, 6, limpar_texto(analise_ia))
    return finalizar_pdf(pdf)

def gerar_pdf_perfil_turma(nome_turma, metricas, analise_ia, lista_alunos):
    """Perfil Individualizado da Turma."""
    pdf = FPDF(); pdf.add_page(); adicionar_cabecalho_diocesano(pdf, f"PERFIL DA TURMA: {nome_turma}")
