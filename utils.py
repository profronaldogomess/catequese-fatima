...
def gerar_relatorio_pastoral_pdf(df_turmas, df_cat, df_pres, df_pres_reuniao):
    ...
    Agora com a Matemática de Nivelamento (Faltas Reais + Pendências de Entrada Tardia).
    """
    pdf = FPDF()
    AZUL_P = (65, 123, 153); LARANJA_P = (224, 61, 17); CINZA_F = (245, 245, 245)
    ...
    for _, t in df_turmas.iterrows():
        ...
        alunos_t = df_cat[df_cat['etapa'].astype(str).str.strip().str.upper() == nome_t]
        if alunos_t.empty: continue
        
        pdf.add_page()
        adicionar_cabecalho_diocesano(pdf, limpar_texto(f"DOSSIE PASTORAL: {nome_t}"))

        # --- 1. PREPARAÇÃO DA MATEMÁTICA DE NIVELAMENTO ---
        ativos = alunos_t[alunos_t['status'] == 'ATIVO']
        pres_t = df_pres[df_pres['id_turma'].astype(str).str.strip().str.upper() == nome_t] if not df_pres.empty else pd.DataFrame()
        
        temas_dados = set()
        if not pres_t.empty and 'tema_do_dia' in pres_t.columns:
            validos = pres_t[~pres_t['tema_do_dia'].str.contains('RECESSO', case=False, na=False)]
            temas_dados = set(validos['tema_do_dia'].dropna().unique())
            
        total_encontros = len(temas_dados)
        
        freq_val = 0.0
        if total_encontros > 0 and len(ativos) > 0:
            total_presencas_turma = len(pres_t[pres_t['status'] == 'PRESENTE'])
            # Frequência Global = Total de Presenças / (Total de Alunos * Total de Encontros Oficiais)
            freq_val = (total_presencas_turma / (len(ativos) * total_encontros)) * 100
        ...
        h_header = num_cats * 6 
        if h_header < 12: h_header = 12 

        pdf.set_fill_color(*AZUL_P); pdf.set_text_color(255, 255, 255); pdf.set_font("helvetica", "B", 10)
        pdf.cell(190, 8, limpar_texto("1. INDICADORES DE ENGAJAMENTO E REGULARIZACAO"), ln=True, fill=True, align='C')
        ...
        pdf.cell(63, h_header, limpar_texto(f"Frequencia Media: {freq_val:.1f}%"), border=1, fill=True, align='C')
        pdf.cell(63, h_header, limpar_texto(f"Documentacao: {docs_ok} / {len(ativos)}"), border=1, fill=True, align='C')
        
        pdf.set_xy(x_start + 126, y_start)
        pdf.multi_cell(64, h_header/num_cats if num_cats > 1 else h_header, limpar_texto(cats_text), border=1, align='C', fill=True)
        
        pdf.set_y(y_start + h_header + 5)

        # --- 2. TABELA NOMINAL DE CAMINHADA ---
        pdf.set_font("helvetica", "B", 10); pdf.set_text_color(*AZUL_P)
        pdf.cell(0, 7, limpar_texto("2. RELACAO NOMINAL E SITUACAO SACRAMENTAL"), ln=True)
        ...
        pdf.cell(90, 7, limpar_texto("Nome Completo do Catequizando"), border=1, fill=True, align='C')
        pdf.cell(30, 7, limpar_texto("Batismo"), border=1, fill=True, align='C')
        pdf.cell(30, 7, limpar_texto("1a Eucaristia"), border=1, fill=True, align='C')
        pdf.cell(30, 7, limpar_texto("Documentacao"), border=1, fill=True, align='C')
        pdf.ln()

        pdf.set_font("helvetica", "", 8)
        nomes_ativos = ativos.sort_values('nome_completo')
        for i, (_, r) in enumerate(nomes_ativos.iterrows(), 1):
            status_bat = "Sim" if r.get('batizado_sn', '') == "SIM" else "Pendente"
            status_euc = "Sim" if "EUCARISTIA" in str(r.get('sacramentos_ja_feitos', '')).upper() else "Pendente"
            status_doc = "Regular" if r.get('doc_em_falta', '') in ['COMPLETO', 'NADA', 'OK', 'NADA FALTANDO'] else "Pendente"
            ...
            pdf.cell(30, altura_linha, limpar_texto(status_bat), border=1, align='C')
            pdf.cell(30, altura_linha, limpar_texto(status_euc), border=1, align='C')
            pdf.cell(30, altura_linha, limpar_texto(status_doc), border=1, align='C')
            pdf.ln()
        ...
        pdf.cell(190, 8, limpar_texto("3. RADAR DE ATENCAO PASTORAL (AGRUPADO)"), ln=True, fill=True, align='C')
        ...
        for _, r in ativos.iterrows():
            idade_c = calcular_idade(r.get('data_nascimento', ''))
            ...
            # MATEMÁTICA DE NIVELAMENTO APLICADA AO RADAR
            pres_aluno = pres_t[pres_t['id_catequizando'] == r['id_catequizando']] if not pres_t.empty else pd.DataFrame()
            qtd_presencas = len(pres_aluno[pres_aluno['status'] == 'PRESENTE']) if not pres_aluno.empty else 0
            faltas_reais = total_encontros - qtd_presencas
            
            if faltas_reais >= 3: 
                risco_evasao.append(f"{r['nome_completo']} ({faltas_reais} faltas/pendencias)")

        tem_alerta = False
        if acima_idade:
            pdf.set_font("helvetica", "B", 9); pdf.write(6, limpar_texto("Catequizandos acima da idade ideal: ")); pdf.set_font("helvetica", "", 9)
            pdf.multi_cell(0, 6, limpar_texto(", ".join(acima_idade)) + "."); tem_alerta = True
        if abaixo_idade:
            pdf.set_font("helvetica", "B", 9); pdf.write(6, limpar_texto("Catequizandos abaixo da idade ideal: ")); pdf.set_font("helvetica", "", 9)
            pdf.multi_cell(0, 6, limpar_texto(", ".join(abaixo_idade)) + "."); tem_alerta = True
        if risco_evasao:
            pdf.set_font("helvetica", "B", 9); pdf.set_text_color(*LARANJA_P); pdf.write(6, limpar_texto("Risco de Evasao ou Nivelamento Pendente (3+): ")); pdf.set_font("helvetica", "", 9)
            pdf.multi_cell(0, 6, limpar_texto(", ".join(risco_evasao)) + "."); pdf.set_text_color(0, 0, 0); tem_alerta = True

        if not tem_alerta:
            pdf.cell(0, 8, limpar_texto("Nenhum alerta critico detectado para esta turma."), ln=True)
...
def obter_ultima_chamada_turma(df_pres, nome_turma):
    ...
    # 1. Normalização rigorosa do nome da turma
    ...
    # 2. Conversão Universal de Datas (Tenta BR e depois ISO)
    def converter_data_flexivel(v):
        v_str = str(v).strip()
        for fmt in ('%d/%m/%Y', '%Y-%m-%d'):
            try: return dt_module.datetime.strptime(v_str.split(' ')[0], fmt)
            except: continue
        return pd.NaT

    pres_t['data_dt'] = pres_t['data_encontro'].apply(converter_data_flexivel)
    pres_t = pres_t.dropna(subset=['data_dt'])
    
    if pres_t.empty: return None, pd.DataFrame()
    
    # 3. Trava de Segurança: Hoje (sem fuso horário)
    hoje_ref = (dt_module.datetime.now(dt_module.timezone.utc) + dt_module.timedelta(hours=-3)).replace(tzinfo=None, hour=23, minute=59, second=59)
    pres_t = pres_t[pres_t['data_dt'] <= hoje_ref]
    
    if pres_t.empty: return None, pd.DataFrame()
    
    # 4. Captura a última data real e filtra
...
def gerar_pdf_auditoria_chamadas(df_turmas, df_pres, df_cat, dias_limite=7):
    """Gera Auditoria focada estritamente no Último Sábado ocorrido."""
    pdf = FPDF()
    pdf.add_page()
    
    # Descobre a data exata do último sábado
    hoje_br_completo = dt_module.datetime.now(dt_module.timezone.utc) + dt_module.timedelta(hours=-3)
    hoje_br = hoje_br_completo.date()
    dias_para_sabado = (hoje_br_completo.weekday() - 5) % 7
    ultimo_sabado = hoje_br - dt_module.timedelta(days=dias_para_sabado)
    
    adicionar_cabecalho_diocesano(pdf, limpar_texto(f"AUDITORIA DE DIARIOS E CHAMADAS (Base: Ultimo Sabado)"))
    
    # --- SESSÃO 1: RESUMO EXECUTIVO ---
    pdf.set_fill_color(240, 242, 246)
    pdf.set_font("helvetica", "B", 10)
    pdf.set_text_color(65, 123, 153)
    pdf.cell(0, 8, limpar_texto(f"1. RESUMO EXECUTIVO DAS TURMAS (Ultimo Sabado: {formatar_data_br(ultimo_sabado)})"), ln=True, fill=True)
    ...
    pdf.cell(20, 7, limpar_texto("Status"), border=1, fill=True, align='C')
    pdf.cell(25, 7, limpar_texto("Ultima"), border=1, fill=True, align='C')
    pdf.cell(95, 7, limpar_texto("Catequista(s)"), border=1, fill=True)
    pdf.ln()
    ...
    for _, t in df_turmas.iterrows():
        ...
        cats_bruto = str(t.get('catequista_responsavel', 'Nao informado'))
        ultima_data, chamada = obter_ultima_chamada_turma(df_pres, nome_t)
        
        # A regra de Ouro: Está em dia SOMENTE se a última chamada for igual ou maior que o último sábado
        if ultima_data and ultima_data >= ultimo_sabado:
            status_resumo = "EM DIA"
            cor_status = (46, 125, 50) # Verde
        else:
            status_resumo = "ATRASADA" if ultima_data else "PENDENTE"
            cor_status = (224, 61, 17) # Vermelho
            
        data_str = formatar_data_br(ultima_data) if ultima_data else "---"
        
        # Lógica de Quebra de Linha para os Catequistas
        cats_list = [c.strip() for c in cats_bruto.split(',') if c.strip()]
        cats_formatado = "\n".join(cats_list)
        linhas_altura = len(cats_list) if len(cats_list) > 0 else 1
        h_row = max(7, linhas_altura * 4) # Calcula altura dinâmica
        
        # Guarda posição X, Y para não atropelar a linha
        x_start = pdf.get_x()
        y_start = pdf.get_y()
        ...
        pdf.cell(50, h_row, limpar_texto(nome_t)[:30], border=1)
        pdf.set_text_color(*cor_status); pdf.set_font("helvetica", "B", 7)
        pdf.cell(20, h_row, status_resumo, border=1, align='C')
        pdf.set_text_color(0, 0, 0); pdf.set_font("helvetica", "", 7)
        pdf.cell(25, h_row, data_str, border=1, align='C')
        
        # O MultiCell faz a quebra do texto. Dividimos a altura pelo nº de linhas para o FPDF entender
        pdf.multi_cell(95, h_row / linhas_altura if linhas_altura > 1 else h_row, limpar_texto(cats_formatado), border=1)
        
        # Força o cursor para a próxima linha exata
        pdf.set_xy(10, y_start + h_row)
        
        tema_dia = "Sem registro"
        if not chamada.empty:
            temas = chamada['tema_do_dia'].dropna().unique()
            if len(temas) > 0: tema_dia = str(temas[0])
            
        turmas_detalhes.append({"nome": nome_t, "status": status_resumo, "data": ultima_data, "tema": tema_dia, "chamada": chamada})
        
    # --- SESSÃO 2: DETALHAMENTO DE FALTOSOS ---
    pdf.ln(5)
    if pdf.get_y() > 240: pdf.add_page()
    pdf.set_fill_color(240, 242, 246); pdf.set_font("helvetica", "B", 10); pdf.set_text_color(65, 123, 153)
    pdf.cell(0, 8, limpar_texto("2. DETALHAMENTO DE FALTOSOS (Apenas turmas EM DIA)"), ln=True, fill=True)
    pdf.ln(2)
    ...
    for td in turmas_detalhes:
        ...
        pdf.set_fill_color(*cor_bg); pdf.set_text_color(255, 255, 255); pdf.set_font("helvetica", "B", 9)
        pdf.cell(0, 7, limpar_texto(f"TURMA: {td['nome']} | Status: {td['status']} | Ultima Chamada: {formatar_data_br(td['data']) if td['data'] else '---'}"), border=1, ln=True, fill=True)
        
        pdf.set_text_color(0, 0, 0)
        
        # SÓ DETALHA SE ESTIVER EM DIA (Se foi feita a chamada do último sábado)
        if td['status'] == "EM DIA":
            pdf.set_font("helvetica", "I", 8); pdf.set_fill_color(245, 245, 245)
            ...
            if not faltosos.empty:
                ...
                for _, f in faltosos.iterrows():
                    ...
                    contatos_str = "Sem contato"
                    if not cat_info.empty:
                        c = cat_info.iloc[0]
                        list_c =[]
                        if str(c.get('tel_mae','')).strip() not in ["N/A","","None"]: list_c.append(f"M:{c['tel_mae']}")
                        if str(c.get('tel_pai','')).strip() not in["N/A","","None"]: list_c.append(f"P:{c['tel_pai']}")
                        if str(c.get('contato_principal','')).strip() not in["N/A","","None"]: list_c.append(f"R:{c['contato_principal']}")
                        if list_c: contatos_str = " | ".join(list_c)
