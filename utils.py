# ARQUIVO: utils.py
# VERSÃO: Auditoria Diocesana e Pastoral V2 (Alta Densidade)
...
# ==========================================
# 3. RELATÓRIO ESTATÍSTICO DIOCESANO (V2)
# ==========================================
...
def gerar_relatorio_diocesano_v2(censo, sacramentos, equipe, analise_ia):
    ...
    # SEÇÃO 2: SACRAMENTOS REALIZADOS NO ANO
    ...
    pdf.cell(70, 7, "Sacramento", border=1, fill=True); pdf.cell(60, 7, "Infantil / Juvenil", border=1, fill=True, align='C')
    pdf.cell(60, 7, "Jovens / Adultos", border=1, fill=True, align='C'); pdf.ln()
    ...
    for nome, k, a in sac_list:
        pdf.cell(70, 7, nome, border=1); pdf.cell(60, 7, str(k), border=1, align='C'); pdf.cell(60, 7, str(a), border=1, align='C'); pdf.ln()
    pdf.ln(5)

    # SEÇÃO 3: PERFIL DA EQUIPE CATEQUÉTICA
    pdf.set_fill_color(65, 123, 153); pdf.set_text_color(255, 255, 255); pdf.set_font("helvetica", "B", 10)
    pdf.cell(190, 8, limpar_texto("3. PERFIL DA EQUIPE (CATEQUISTAS)"), ln=True, fill=True, align='C')
    pdf.set_font("helvetica", "B", 8); pdf.set_text_color(0, 0, 0); pdf.set_fill_color(230, 230, 230)
    pdf.cell(38, 7, "Total Equipe", border=1, fill=True, align='C'); pdf.cell(38, 7, "Batizados", border=1, fill=True, align='C')
    pdf.cell(38, 7, "Crismados", border=1, fill=True, align='C'); pdf.cell(38, 7, "Aptos", border=1, fill=True, align='C')
    pdf.cell(38, 7, "Ministros", border=1, fill=True, align='C'); pdf.ln()
    pdf.set_font("helvetica", "", 9)
    pdf.cell(38, 7, str(equipe['total']), border=1, align='C'); pdf.cell(38, 7, str(equipe['bat']), border=1, align='C')
    pdf.cell(38, 7, str(equipe['cris']), border=1, align='C'); pdf.cell(38, 7, str(equipe['apto']), border=1, align='C')
    pdf.cell(38, 7, str(equipe['min']), border=1, align='C'); pdf.ln(12)

    # SEÇÃO 4: PARECER TÉCNICO IA
...
# ==========================================
# 4. RELATÓRIO PASTORAL INTERNO (V2)
# ==========================================
...
def gerar_relatorio_pastoral_v2(turmas_data, analise_ia):
    ...
    pdf.cell(45, 7, "Turma / Etapa", border=1, fill=True); pdf.cell(40, 7, "Catequista(s)", border=1, fill=True)
    pdf.cell(35, 7, "Local / Dia", border=1, fill=True); pdf.cell(20, 7, "Catequiz.", border=1, fill=True, align='C')
    pdf.cell(25, 7, "Batizados", border=1, fill=True, align='C'); pdf.cell(25, 7, "Eucaristia", border=1, fill=True, align='C'); pdf.ln()
    ...
    for t in turmas_data:
        ...
        pdf.cell(25, 6, str(t['eucaristia']), border=1, align='C'); pdf.ln()
...
# ==========================================
# 5. FUNÇÕES ORIGINAIS (RESTAURADAS)
# ==========================================
...
def eh_aniversariante_da_semana(data_nasc_str):
    try:
        d_str = formatar_data_br(data_nasc_str); if d_str == "N/A": return False
        nasc = datetime.strptime(d_str, "%d/%m/%Y").date(); hoje = (datetime.now(timezone.utc) + timedelta(hours=-3)).date()
        nasc_este_ano = nasc.replace(year=hoje.year); return 0 <= (nasc_este_ano - hoje).days <= 7
    except: return False
