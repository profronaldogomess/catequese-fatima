...
def gerar_relatorio_diocesano_v4(df_turmas, df_cat, df_usuarios):
    """
    Versão Analítica Final: Itinerários corrigidos, Censo de Cobertura, 
...
def gerar_relatorio_pastoral_v3(df_turmas, df_cat, df_pres):
    ...
    total_geral_cat = 0; total_geral_bat = 0; total_geral_euc = 0; total_geral_cri = 0
    soma_frequencias = 0; qtd_turmas_com_presenca = 0
    ...
    for _, t in df_turmas.iterrows():
        ...
        total_geral_cat += total_t; total_geral_bat += bat_t; total_geral_euc += euc_t; total_geral_cri += cri_t
        ...
        pdf.cell(38, 7, "Total", border=1, fill=True, align='C'); pdf.ln()
        ...
        pdf.cell(38, 7, str(total_t), border=1, align='C'); pdf.ln(8)

        pdf.set_font("helvetica", "B", 9)
        pdf.cell(0, 5, limpar_texto(f"Catequista(s): {t['catequista_responsavel']}"), ln=True); pdf.ln(2)
        ...
        for i in range(0, len(nomes_lista), 2):
            pdf.cell(95, 5, limpar_texto(f"  - {nomes_lista[i]}"), border=0)
            if i + 1 < len(nomes_lista):
                pdf.cell(95, 5, limpar_texto(f"  - {nomes_lista[i+1]}"), border=0)
            pdf.ln()
    ...
    pdf.add_page()
    pdf.set_fill_color(*AZUL_P); pdf.set_text_color(255, 255, 255); pdf.set_font("helvetica", "B", 12)
    ...
    for desc, valor in resumo_final:
        ...
        pdf.cell(60, 10, str(valor), border=1, align='C'); pdf.ln()
...
def gerar_relatorio_sacramentos_tecnico_v2(stats_gerais, analise_turmas, impedimentos_lista, analise_ia):
    ...
    pdf.cell(70, 7, "Jovens / Adultos", border=1, fill=True, align='C'); pdf.ln()
    ...
    for sac, k, a in censo_data:
        ...
        pdf.cell(70, 7, str(a), border=1, align='C'); pdf.ln()
    ...
    pdf.cell(70, 7, "Situação Matrimonial / Sacramental", border=1, fill=True); pdf.ln()
    ...
    if impedimentos_lista:
        for imp in impedimentos_lista:
            ...
            pdf.cell(70, 6, limpar_texto(imp.get('situacao', 'N/A')), border=1); pdf.ln()
    else:
