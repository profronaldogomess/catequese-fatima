...
def gerar_relatorio_diocesano_v4(df_turmas, df_cat, df_usuarios):
    """
    Versão Analítica Completa e Extensa: Itinerários corrigidos, Censo de Cobertura, 
    Frutos Nominais 2026 e Qualificação Detalhada da Equipe.
    """
    ...
    def eh_infantil(row):
        ...
        # Se contiver PRÉ, ETAPA ou PERSEVERANÇA, é infantil, a menos que diga ADULTO
    ...
    if not df_turmas.empty:
        ...
        t_infantil = t_adultos = pd.DataFrame()

    # --- TABELA 1: ITINERÁRIOS INFANTIL / JUVENIL ---
    pdf.set_fill_color(*AZUL_P)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(190, 8, limpar_texto(f"1. ITINERÁRIOS INFANTIL / JUVENIL ({len(t_infantil)} turmas)"), ln=True, fill=True, align='C')
    
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("helvetica", "B", 8)
    pdf.set_fill_color(*CINZA_F)
    pdf.cell(60, 7, "Nome da Turma", border=1, fill=True)
    pdf.cell(65, 7, "Catequista Responsável", border=1, fill=True)
    pdf.cell(22, 7, "Batizados", border=1, fill=True, align='C')
    pdf.cell(22, 7, "Eucaristia", border=1, fill=True, align='C')
    pdf.cell(21, 7, "Total", border=1, fill=True, align='C')
    pdf.ln()

    pdf.set_font("helvetica", "", 8)
    for _, t in t_infantil.iterrows():
        ...
        pdf.cell(60, 7, limpar_texto(t['nome_turma']), border=1)
        pdf.cell(65, 7, limpar_texto(t['catequista_responsavel']), border=1)
        pdf.cell(22, 7, str(bat), border=1, align='C')
        pdf.cell(22, 7, str(euc), border=1, align='C')
        pdf.cell(21, 7, str(len(alunos)), border=1, align='C')
        pdf.ln()

    pdf.ln(5)

    # --- TABELA 2: ITINERÁRIOS DE JOVENS E ADULTOS ---
    pdf.set_fill_color(*AZUL_P)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(190, 8, limpar_texto(f"2. ITINERÁRIOS DE JOVENS E ADULTOS ({len(t_adultos)} turmas)"), ln=True, fill=True, align='C')
    ...
    # Cálculo de idade para separação de censo
    df_cat['idade_temp'] = df_cat['data_nascimento'].apply(calcular_idade)
    df_k = df_cat[df_cat['idade_temp'] < 18]
    df_a = df_cat[df_cat['idade_temp'] >= 18]
    
    for label, df_grupo in [("Público Infantil/Juvenil", df_k), ("Público Jovens/Adultos", df_a)]:
        if not df_grupo.empty:
            total = len(df_grupo)
            bat = len(df_grupo[df_grupo['batizado_sn'] == 'SIM'])
            perc = (bat / total) * 100 if total > 0 else 0
    ...
    if not df_rec.empty:
        ...
        if not df_ano.empty:
            ...
            pdf.set_font("helvetica", "B", 8)
            pdf.set_fill_color(*CINZA_F)
            pdf.cell(100, 7, "Nome do Catequizando", border=1, fill=True)
            pdf.cell(50, 7, "Sacramento", border=1, fill=True, align='C')
            pdf.cell(40, 7, "Data", border=1, fill=True, align='C')
            pdf.ln()
            ...
            for _, r in df_ano.iterrows():
                ...
                pdf.cell(40, 6, formatar_data_br(r['data']), border=1, align='C')
                pdf.ln()

    # --- 5. EQUIPE CATEQUÉTICA E QUALIFICAÇÃO ---
    pdf.ln(5)
    pdf.set_fill_color(*AZUL_P)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", "B", 10)
    ...
    for _, u in df_usuarios.iterrows():
        status, _ = verificar_status_ministerial(
            u.get('data_inicio_catequese',''), 
            u.get('data_batismo',''), 
            u.get('data_eucaristia',''), 
            u.get('data_crisma',''), 
            u.get('data_ministerio','')
        )
        if status in ["APTO", "MINISTRO"]:
            aptos += 1
    ...
    indicadores_equipe = [
        ...
        ("Eucaristia", euc_e),
        ("Crisma", cri_e),
        ("Aptos para o Ministério", aptos)
    ]
    
    for desc, qtd in indicadores_equipe:
        pdf.cell(100, 6, f" {desc}", border=1)
        pdf.cell(45, 6, str(qtd), border=1, align='C')
        pdf.cell(45, 6, f"{(qtd/total_e)*100:.1f}%", border=1, align='C')
        pdf.ln()

    # Lista Nominal Final da Equipe
    pdf.ln(2)
    pdf.set_font("helvetica", "B", 8)
    pdf.cell(0, 5, "Membros da Equipe:", ln=True)
    pdf.set_font("helvetica", "", 7)
    nomes_equipe = df_usuarios['nome'].tolist()
...
def gerar_relatorio_pastoral_v3(df_turmas, df_cat, df_pres):
    """
    Dossiê Pastoral Nominal: Detalhamento por turma com estatísticas,
    catequistas responsáveis e lista nominal completa de catequizandos.
    """
    pdf = FPDF()
    pdf.add_page()
    adicionar_cabecalho_diocesano(pdf, "RELATÓRIO PASTORAL E NOMINAL POR ITINERÁRIO")
    ...
    total_geral_cri = 0
    soma_frequencias = 0
    qtd_turmas_com_presenca = 0

    if df_turmas.empty:
        pdf.cell(0, 10, "Nenhuma turma cadastrada.", ln=True)
        return finalizar_pdf(pdf)
    ...
    for _, t in df_turmas.iterrows():
        ...
        # Cálculos da Turma
        total_t = len(alunos_t)
        bat_t = len(alunos_t[alunos_t['batizado_sn'] == 'SIM'])
        ...
        if not pres_t.empty:
            freq_t = (pres_t['status'].value_counts(normalize=True).get('PRESENTE', 0) * 100)
            soma_frequencias += freq_t
            qtd_turmas_com_presenca += 1

        total_geral_cat += total_t
        total_geral_bat += bat_t
        total_geral_euc += euc_t
        total_geral_cri += cri_t
        ...
        pdf.cell(38, 7, "Eucaristia", border=1, fill=True, align='C')
        pdf.cell(38, 7, "Crisma", border=1, fill=True, align='C')
        pdf.cell(38, 7, "Freq. Média", border=1, fill=True, align='C')
        pdf.cell(38, 7, "Total Inscritos", border=1, fill=True, align='C')
        pdf.ln()
        
        pdf.set_font("helvetica", "", 9)
        pdf.cell(38, 7, str(bat_t), border=1, align='C')
        pdf.cell(38, 7, str(euc_t), border=1, align='C')
        pdf.cell(38, 7, str(cri_t), border=1, align='C')
        pdf.cell(38, 7, f"{freq_t:.1f}%", border=1, align='C')
        pdf.cell(38, 7, str(total_t), border=1, align='C')
        pdf.ln(8)

        # Catequistas
        pdf.set_font("helvetica", "B", 9)
        pdf.cell(0, 5, limpar_texto(f"Catequista(s): {t['catequista_responsavel']}"), ln=True)
        pdf.ln(2)

        # Lista Nominal (2 Colunas)
        pdf.set_font("helvetica", "B", 8)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 5, "LISTA NOMINAL DE CATEQUIZANDOS:", ln=True)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("helvetica", "", 8)
        
        nomes_lista = sorted(alunos_t['nome_completo'].tolist())
        col_w = 95
        for i in range(0, len(nomes_lista), 2):
            nome1 = nomes_lista[i]
            pdf.cell(col_w, 5, limpar_texto(f"  - {nome1}"), border=0)
            if i + 1 < len(nomes_lista):
                nome2 = nomes_lista[i+1]
                pdf.cell(col_w, 5, limpar_texto(f"  - {nome2}"), border=0)
            pdf.ln()
    ...
    resumo_final = [
        ("Total de Catequizandos Matriculados", total_geral_cat),
        ("Total de Batizados", total_geral_bat),
        ("Total com Primeira Eucaristia", total_geral_euc),
        ("Total com Crisma", total_geral_cri),
        ("Frequência Média Paroquial", f"{(soma_frequencias/qtd_turmas_com_presenca if qtd_turmas_com_presenca > 0 else 0):.1f}%")
    ]
    
    for desc, valor in resumo_final:
        pdf.cell(130, 10, limpar_texto(desc), border=1, fill=True)
        pdf.cell(60, 10, str(valor), border=1, align='C')
        pdf.ln()
...
def gerar_relatorio_sacramentos_tecnico_v2(stats_gerais, analise_turmas, impedimentos_lista, analise_ia):
    """
    Gera a Auditoria Sacramental Técnica.
    Analisa o Censo de IVC e mapeia impedimentos canônicos nominais.
    """
    pdf = FPDF()
    pdf.add_page()
    ...
    # --- 1. QUADRO GERAL DE SACRAMENTALIZAÇÃO ---
    pdf.set_fill_color(*AZUL_P)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(190, 8, limpar_texto("1. QUADRO GERAL DE SACRAMENTALIZAÇÃO (CENSO REAL)"), ln=True, fill=True, align='C')
    ...
    pdf.set_font("helvetica", "", 9)
    censo_data = [
        ("BATISMO", stats_gerais.get('bat_k', 0), stats_gerais.get('bat_a', 0)),
        ("EUCARISTIA", stats_gerais.get('euca_k', 0), stats_gerais.get('euca_a', 0)),
        ("CRISMA", "N/A", stats_gerais.get('crisma_a', 0))
    ]
    
    for sac, k, a in censo_data:
        pdf.cell(50, 7, f" {sac}", border=1)
    ...
    # --- 2. DIAGNÓSTICO DE IMPEDIMENTOS CANÔNICOS ---
    pdf.set_fill_color(*AZUL_P)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", "B", 10)
    pdf.cell(190, 8, limpar_texto("2. DIAGNÓSTICO DE IMPEDIMENTOS E REGULARIZAÇÕES"), ln=True, fill=True, align='C')
    ...
    if impedimentos_lista:
        for imp in impedimentos_lista:
            pdf.cell(70, 6, limpar_texto(imp.get('nome', 'N/A')), border=1)
            pdf.cell(50, 6, limpar_texto(imp.get('turma', 'N/A')), border=1)
            pdf.cell(70, 6, limpar_texto(imp.get('situacao', 'N/A')), border=1)
            pdf.ln()
    else:
        pdf.cell(190, 7, "Nenhum impedimento crítico registrado no momento.", border=1, align='C', ln=True)

    pdf.ln(5)

    # --- 3. PARECER TÉCNICO PASTORAL ---
    pdf.set_fill_color(*LARANJA_P)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font("helvetica", "B", 10)
...
def gerar_relatorio_local_turma_v2(nome_turma, metricas, listas, analise_ia):
    """
    Gera o relatório de Auditoria Pastoral para uma turma específica.
    Inclui indicadores, evolução mensal, lista de faltas e sacramentos.
    """
    pdf = FPDF()
    pdf.add_page()
    ...
    # --- 1. INDICADORES ESTRUTURAIS E ADESÃO ---
    ...
    pdf.set_text_color(0, 0, 0); y = pdf.get_y() + 2
    desenhar_campo_box(pdf, "Catequistas na Turma", str(metricas.get('qtd_catequistas', 0)), 10, y, 45)
    desenhar_campo_box(pdf, "Total Catequizandos", str(metricas.get('qtd_cat', 0)), 58, y, 45)
    desenhar_campo_box(pdf, "Frequência Global", f"{metricas.get('freq_global', 0)}%", 106, y, 45)
    desenhar_campo_box(pdf, "Idade Média", f"{metricas.get('idade_media', 0)} anos", 154, y, 46)
    pdf.ln(18)
    ...
    pdf.cell(95, 7, "Taxa de Presença", border=1, fill=True, align='C'); pdf.ln()
    
    pdf.set_font("helvetica", "", 9)
    freq_mensal = metricas.get('freq_mensal', [])
    if freq_mensal:
        for m in freq_mensal:
            pdf.cell(95, 6, limpar_texto(m['mes']), border=1, align='C')
            pdf.cell(95, 6, f"{m['taxa']}%", border=1, align='C'); pdf.ln()
    else:
        pdf.cell(190, 6, "Sem dados de frequência registrados.", border=1, align='C', ln=True)
    ...
    pdf.cell(70, 7, "Status / Faltas", border=1, fill=True, align='C'); pdf.ln()
    ...
    pdf.cell(50, 7, "Sacramento", border=1, fill=True, align='C')
    pdf.cell(60, 7, "Data do Registro", border=1, fill=True, align='C'); pdf.ln()
    
    pdf.set_font("helvetica", "", 8)
    sac_recebidos = listas.get('sac_recebidos', [])
    if sac_recebidos:
        for s in sac_recebidos:
    ...
    pdf.ln(5); pdf.set_fill_color(*AZUL_P); pdf.set_text_color(255, 255, 255)
...
def gerar_auditoria_lote_completa(df_turmas, df_cat, df_pres, df_recebidos):
    ...
    for _, t in df_turmas.iterrows():
        ...
        if not alunos_t.empty:
            ...
            freq_g = 0.0
            if not pres_t.empty:
                freq_g = round(pres_t['status'].apply(lambda x: 1 if x == 'PRESENTE' else 0).mean() * 100, 1)
            ...
            for _, r in alunos_t.iterrows():
                faltas = 0
                if not pres_t.empty and col_id_cat in pres_t.columns:
                    faltas = len(pres_t[(pres_t[col_id_cat] == r[col_id_cat]) & (pres_t['status'] == 'AUSENTE')])
