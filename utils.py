...
def gerar_relatorio_diocesano_v5(df_turmas, df_cat, df_usuarios):
    ...
    # 2. RESUMO DE CELEBRAÇÕES (DADOS REAIS)
    ...
    if not df_eventos.empty:
        ...
        if not df_ano_ev.empty:
            ...
            pdf.cell(190, 8, limpar_texto(f"3. RESUMO DE CELEBRAÇÕES REALIZADAS EM {ANO_ATUAL}"), ln=True, fill=True, align='C')
            pdf.set_text_color(0, 0, 0); pdf.set_font("helvetica", "B", 8); pdf.set_fill_color(*CINZA_F)
            pdf.cell(80, 7, "Turma / Itinerário", border=1, fill=True); pdf.cell(40, 7, "Sacramento", border=1, fill=True, align='C'); pdf.cell(40, 7, "Data", border=1, fill=True, align='C'); pdf.cell(30, 7, "Qtd", border=1, fill=True, align='C'); pdf.ln()
            pdf.set_font("helvetica", "", 8)
    ...
    pdf.cell(190, 8, limpar_texto(f"4. EQUIPE CATEQUÉTICA (Total: {len(df_equipe_real)})"), ln=True, fill=True, align='C')
    pdf.set_text_color(0, 0, 0); pdf.set_font("helvetica", "B", 8); pdf.set_fill_color(*CINZA_F)
    ...
    pdf.cell(100, 6, " Batismo", border=1); pdf.cell(45, 6, str(bat_e), border=1, align='C'); pdf.cell(45, 6, f"{(bat_e/total_e)*100:.1f}%", border=1, align='C'); pdf.ln()
